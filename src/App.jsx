...
async function fetchAllMatches(){try{const j=await getJSON(`${ESPN_S}/scoreboard?dates=${WC_RANGE}&limit=400`);const out=[];let id=0;const scorerMap={};for(const ev of (j.events||[])){const comp=ev.competitions&&ev.competitions[0];if(!comp)continue;const cs=comp.competitors||[];const hC=cs.find(c=>c.homeAway==='home')||cs[0],aC=cs.find(c=>c.homeAway==='away')||cs[1];const home=ingestTeam(hC&&hC.team),away=ingestTeam(aC&&aC.team);if(!home||!away||home===away)continue;const st=ev.status&&ev.status.type&&ev.status.type.state;const status=st==='post'?'finished':st==='in'?'live':'upcoming';const minute=status==='live'?(parseClock(ev.status&&ev.status.displayClock).min|0||1):null;const round=parseRound(comp,ev)||dateRound(ev.date);const m={id:++id,espnId:ev.id,round,group:(teams[home]&&teams[home].group)||'—',home,away,datetime:ev.date,venue:(comp.venue&&comp.venue.fullName)||venues[id%venues.length],status,hs:null,as:null,hpen:null,apen:null,minute,events:[],stats:null,lineups:null,_detail:false};if(status!=='upcoming'){m.hs=parseInt(hC.score)||0;m.as=parseInt(aC.score)||0;const hp=hC.shootoutScore,ap=aC.shootoutScore;if(hp!=null&&ap!=null&&!(hp===0&&ap===0)){m.hpen=parseInt(hp)||0;m.apen=parseInt(ap)||0;}const det=parseCompDetails(comp,home,away,scorerMap);m.events=det.length?det:genEvents(home,away,m.hs,m.as,status==='live'?(minute||45):90);m.stats=genStats(m);}out.push(m);}const scorers=Object.values(scorerMap).filter(s=>s.goals>0).sort((a,b)=>b.goals-a.goals||b.assists-a.assists).slice(0,40);return{ok:true,matches:out,scorers};}catch(e){return{ok:false,matches:[],scorers:[]};}}
...
function MatchCard({m,onMatch,onTeam}){const g=groupOf(m.home);const hasPen=typeof m.hpen==='number'&&typeof m.apen==='number';return (
...
/* ============================================================ 淘汰賽對陣表（動態：上一輪 → 顯示「A 或 B 勝方」；再上一輪 → 待定。支援加時／互射十二碼） ============================================================ */
const leftR32=[
...
function koWinner(m){
  ...
  if(typeof m.hpen==='number'&&typeof m.apen==='number'){
    if(m.hpen>m.apen)return m.home;
    if(m.apen>m.hpen)return m.away;
  }
  return null;
}
function koLoser(m){
...
function KORow({code,score,pen,win,show,showPen}){const known=teams[code]&&(teams[code].iso||teams[code].logo);return (<div className="flex items-center justify-between gap-1 py-0.5"><span className="flex items-center gap-1 min-w-0"><Flag code={code} cls="w-4 h-3 shrink-0"/><span className={`truncate text-[11px] ${win?'font-extrabold text-slate-900':'text-slate-600'}`}>{known?tname(code):'待定'}</span></span>{show&&<span className="flex items-center gap-1 shrink-0">{showPen&&<span className={`text-[9px] tabular-nums ${win?'text-emerald-600 font-bold':'text-slate-400'}`}>({pen})</span>}<span className={`text-[11px] font-bold tabular-nums ${win?'text-emerald-600':'text-slate-400'}`}>{score}</span></span>}</div>);}
...
function KOCell({cell,onMatch,gold,time}){
  ...
  if(m){
    const live=m.status==='live',done=m.status==='finished',show=live||done;
    const w=done?koWinner(m):null,wH=w===m.home,wA=w===m.away;
    const hasPen=typeof m.hpen==='number'&&typeof m.apen==='number';
    return (<button onClick={()=>onMatch(m)} className={`w-full rounded-lg border ${gold?'border-amber-400 ring-2 ring-amber-300':'border-white'} bg-white shadow-md hover:bg-slate-50 px-2 py-1.5 text-left transition`}>
      <div className="flex items-center justify-between text-[9px] mb-1 leading-none">
        {live?<span className="flex items-center gap-1 text-rose-600 font-bold"><span className="w-1.5 h-1.5 rounded-full bg-rose-500 animate-pulse"/>{m.minute}'</span>:done?<span className="text-slate-400 font-semibold">{hasPen?'點球完場':'完場'}</span>:<span className="text-slate-500 font-medium">{fmtKO(m.datetime)} {fmtTime(m.datetime)}</span>}
      </div>
      <KORow code={m.home} score={m.hs} pen={m.hpen} win={wH} show={show} showPen={hasPen}/>
      <KORow code={m.away} score={m.as} pen={m.apen} win={wA} show={show} showPen={hasPen}/>
    </button>);
  }
...
return (<div className="relative rounded-3xl overflow-hidden shadow-xl" style={{background:'linear-gradient(135deg,#0f2a5c 0%,#1e3a8a 45%,#0c2347 100%)'}}>
  ...
  <div className="relative p-5">
    ...
    <p className="text-xs mb-1" style={{color:'rgba(255,255,255,0.8)'}}>對陣依官方賽程排定。每場分出勝負後，下一輪會自動顯示晉級者；再下一輪則先以「○ 或 ○ 勝方」標示，更遠輪次顯示待定。打和則以互射十二碼比分判定晉級。日期時間均為香港時間（GMT+8）。</p>
